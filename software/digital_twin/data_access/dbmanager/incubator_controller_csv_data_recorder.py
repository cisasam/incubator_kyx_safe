...
from incubator.communication.shared.protocol import ROUTING_KEY_CONTROLLER


class IncubatorControllerDataRecorderCSV:
    ...
    def read_state(self, ch, method, properties, body_json):
        ...
        if self.current_file_name is None or self.number_records >= self.rollover_limit:
            ...
            self.current_file_name = datetime.now().strftime(f"{self.csv_file_prefix}_%Y-%m-%d__%H_%M_%S_cont.csv")
    ...
    def start_recording(self, rabbit_config):
        ...
        try:
            rabbitmq.subscribe(routing_key=ROUTING_KEY_CONTROLLER,
